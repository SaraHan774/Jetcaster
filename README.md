...
This is implemented in [`DynamicTheming.kt`](app/src/main/java/com/august/jetcaster/util/DynamicTheming.kt), which provides the `DynamicThemePrimaryColorsFromImage` composable, to automatically animate the theme colors based on the provided image URL, like so:
...
Underneath, [`DominantColorState`](app/src/main/java/com/august/jetcaster/util/DynamicTheming.kt) uses the [Coil][coil] library to fetch the artwork image 🖼️, and then [Palette][palette] to extract the dominant colors from the image 🎨.
...
Using the example of the home screen in the [`com.example.jetcaster.ui.home`](app/src/main/java/com/august/jetcaster/ui/home) package:
...
- __Home:__ [`com.example.jetcaster.ui.home`](app/src/main/java/com/august/jetcaster/ui/home)
- __Discover:__ [`com.example.jetcaster.ui.home.discover`](app/src/main/java/com/august/jetcaster/ui/home/discover)
- __Podcast Category:__ [`com.example.jetcaster.ui.category`](app/src/main/java/com/august/jetcaster/ui/home/category)

## Data

### Podcast data

The podcast data in this sample is dynamically fetched from a number of podcast RSS feeds, which are listed in [`Feeds.kt`](app/src/main/java/com/august/jetcaster/data/Feeds.kt). 
...
The [`PodcastRepository`][podcastrepo] class is responsible for handling the data fetching of all podcast information:
 ...
 The sample allows users to 'follow' podcasts, which is implemented within the data layer in the [`PodcastFollowedEntry`](app/src/main/java/com/august/jetcaster/data/PodcastFollowedEntry.kt) entity class, and as functions in [PodcastStore][podcaststore]: `followPodcast()`, `unfollowPodcast()`.

 ### Date + time

 The sample uses the JDK 8 [date and time APIs](https://developer.android.com/reference/java/time/package-summary) through the [desugaring support][jdk8desugar] available in Android Gradle Plugin 4.0+. Relevant Room [`TypeConverters`](https://developer.android.com/reference/kotlin/androidx/room/TypeConverters) are implemented in [`DateTimeTypeConverters.kt`](app/src/main/java/com/august/jetcaster/data/room/DateTimeTypeConverters.kt).
...
```
 ...
 [feeds]: app/src/main/java/com/august/jetcaster/data/Feeds.kt
 [fetcher]: app/src/main/java/com/august/jetcaster/data/PodcastFetcher.kt
 [podcastrepo]: app/src/main/java/com/august/jetcaster/data/PodcastsRepository.kt
 [podcaststore]: app/src/main/java/com/august/jetcaster/data/PodcastStore.kt
 [epstore]: app/src/main/java/com/august/jetcaster/data/EpisodeStore.kt
 [catstore]: app/src/main/java/com/august/jetcaster/data/CategoryStore.kt
 [db]: app/src/main/java/com/august/jetcaster/data/room/JetcasterDatabase.kt
 [homevm]: app/src/main/java/com/august/jetcaster/ui/home/HomeViewModel.kt
 [homeui]: app/src/main/java/com/august/jetcaster/ui/home/Home.kt
